...
if(~exist('axisSwitchXX','var'))
    axisSwitchXX = uicontrol('Style','checkbox','String','Xa axis','pos',[300 20 50 23],'parent',figureHandle);
end

if(~exist('axisSwitchYY','var'))
    axisSwitchYY = uicontrol('Style','checkbox','String','Ya axis','pos',[350 20 50 23],'parent',figureHandle);
end
if(~exist('axisSwitchZZ','var'))
    axisSwitchZZ = uicontrol('Style','checkbox','String','Za axis','pos',[400 20 50 23],'parent',figureHandle);
end
%{
...
if(~exist('degreeLabelX','var'))
    degreeLabelX = uicontrol('Style','checkbox','String','X: 0 m/s','pos',[450 100 100 23],'parent',figureHandle);
end

if(~exist('degreeLabelY','var'))
    degreeLabelY= uicontrol('Style','checkbox','String','Y:0 m/s','pos',[450 75 100 23],'parent',figureHandle);
end
if(~exist('degreeLabelZ','var'))
    degreeLabelZ= uicontrol('Style','checkbox','String','Z: 0 m/s','pos',[450 50 100 23],'parent',figureHandle);
end
%}
if(~exist('degreeLabelXX','var'))
    degreeLabelXX = uicontrol('Style','checkbox','String','X: 0 m/s^2','pos',[450 150 100 23],'parent',figureHandle);
end

if(~exist('degreeLabelYY','var'))
    degreeLabelYY= uicontrol('Style','checkbox','String','Y:0 m/s^2','pos',[450 200 100 23],'parent',figureHandle);
end
if(~exist('degreeLabelZZ','var'))
    degreeLabelZZ= uicontrol('Style','checkbox','String','Z: 0 m/s^2','pos',[450 250 100 23],'parent',figureHandle);
end
...
while( get(stopButton,'Value') ==0)
    tic;
    [AngleX, AngleY,AngleZ] = trial_velocity(gyroConnection_s);
    current = now;
    X= X+current*AngleX;
    Y=Y+current*AngleY;
    Z=Z+current*AngleZ;
    ...
    if get(axisSwitchZ,'Value') ==0
     ...
     if get(axisSwitchXX,'Value') ==0
        X =0;
     end
    
    if get(axisSwitchYY,'Value') ==0
        Y =0;
    end
    if get(axisSwitchZZ,'Value') ==0
        Z = 0;
     end
    ...
    set(degreeLabelXX,'String',['X: ' num2str((AngleX)) 'm/s^2'])
    set(degreeLabelYY,'String',['Y: ' num2str((AngleY)) 'm/s^2'])
    set(degreeLabelZZ,'String',['Z: ' num2str((AngleZ)) 'm/s^2'])
